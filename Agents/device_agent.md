...
## 📥 Input Params
* `NormalizedTransaction` containing `device_id` and `customer_id`.
...
* `anomalies`: List of flagged anomalies (e.g. `shared_device_4_customers`).
* `evidence`: Contains trust level logs.
